...
import (
	...
	apisCluster "github.com/wildbitca/provider-upjet-supabase/apis/cluster"
	apisNamespaced "github.com/wildbitca/provider-upjet-supabase/apis/namespaced"
	"github.com/wildbitca/provider-upjet-supabase/config"
	"github.com/wildbitca/provider-upjet-supabase/internal/clients"
	controllerCluster "github.com/wildbitca/provider-upjet-supabase/internal/controller/cluster"
	controllerNamespaced "github.com/wildbitca/provider-upjet-supabase/internal/controller/namespaced"
	"github.com/wildbitca/provider-upjet-supabase/internal/features"
	"github.com/wildbitca/provider-upjet-supabase/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for UpjetSupabase").DefaultEnvars()
		debug                   = app.Flag("debug", "Run with debug logging.").Short('d').Bool()
		syncPeriod              = app.Flag("sync", "Controller manager sync period such as 300ms, 1.5h, or 2h45m").Short('s').Default("1h").Duration()
		pollInterval            = app.Flag("poll", "Poll interval controls how often an individual resource should be checked for drift.").Default("10m").Duration()
		pollStateMetricInterval = app.Flag("poll-state-metric", "State metric recording interval").Default("5s").Duration()
		leaderElection          = app.Flag("leader-election", "Use leader election for the controller manager.").Short('l').Default("false").OverrideDefaultFromEnvar("LEADER_ELECTION").Bool()
		maxReconcileRate        = app.Flag("max-reconcile-rate", "The global maximum rate per second at which resources may be checked for drift from the desired state.").Default("3").Int()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-supabase"))
	if *debug {
		ctrl.SetLogger(zl)
	}
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-supabase-function",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped UpjetSupabase APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced UpjetSupabase APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-upjet-supabase:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated_function(mgr, clusterOpts), "Cannot setup cluster-scoped UpjetSupabase controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated_function(mgr, namespacedOpts), "Cannot setup namespaced UpjetSupabase controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup_function(mgr, clusterOpts), "Cannot setup cluster-scoped UpjetSupabase controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup_function(mgr, namespacedOpts), "Cannot setup namespaced UpjetSupabase controllers")
	}
